...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn, vector<int> & pointsRtn) {
	bool intersects = false;
	if (node.children.size() == 0) {
		boxListRtn.push_back(node.box);
		pointsRtn.push_back(node.points[0]);
		return true;
	}
	if (node.box.overlap(box)) {
		...
		//boxListRtn.push_back(node.box);
		for (int i = 0; i < node.children.size(); i++) {
			intersect(box, node.children[i], boxListRtn, pointsRtn);
		}
	};
	return intersects;
}
